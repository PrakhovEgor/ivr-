...
def main():
    ...
    if request.method == "POST":
        if "logout" in request.form:
            ...
            return redirect("/login/")
        elif "date_picker" in request.form:
...
def edit(user):
    ...
    if request.method == "POST":
        if "logout" in request.form:
            ...
            plants = Plants_().get_plants_idname(search=search, id=Plants_().get_user_plants_id())
            ...
            User_().save_plants_to_user(plants_to_save)
            plants_lst = add_padding(plants_id, 2)
            User_().save_action(plants_lst, "watering")
            return redirect("/")
            ...
            seeds = add_padding(seeds, 1)
            sprouts = add_padding(sprouts, 1)
            User_().save_action(seeds, "planting_seeds")
            User_().save_action(sprouts, "planting_sprouts")
            return redirect("/")
            ...
            for el in temp:
                User_().save_action([[el[0]] + ['0', '0']], "tending_" + el[1])
            return redirect("/")
            ...
            User_().save_action(temp, "harvesting")
            return redirect("/")
    try:
...
def login():
    ...
    if request.method == "POST":
        if form.validate():
            ...
            elif password != session.get("User.user", User(mysql=mysql)).get_user_password(email):
                password_error = True
            else:
                session["email"] = email
...
def user_plants():
    ...
    user_plants = Plants_().get_plants_idname(id=Plants_().get_user_plants_id(), img=True)
    for i in range(len(user_plants)):
        if os.path.isdir('static/' + session.get('email')):
            for j in os.listdir('static/' + session.get('email')):
                if user_plants[i][3] == j:
                    user_plants[i][3] = session.get('email') + '/' + user_plants[i][3]
...
def make_reminder():
    ...
    plants = Plants_().get_plants_idname(search=search, id=Plants_().get_user_plants_id())
    accounts = User_().get_user_tg(id=True)
    return render_template("make_reminders.html", plants=plants, plant_input_placeholder=plant_input_placeholder,
                           ...
                           timedelta=datetime.timedelta(days=1), accounts=accounts)
...
def constructor():
    ...
    beds = User_().get_beds()
    return render_template("constructor.html", active_cg="text-white", main="Журнал", name_error=name_error, user=session.get('email'), beds=beds, test=123, chart_data=123)
...
def make_bed(width, height, name, id=None):
    ...
    plants = Plants_().get_plants_idname(id=Plants_().get_user_plants_id(), img=True)
    print(data)
...
def plant(plant_id):
    ...
    name = info[1]
    if os.path.isdir('static/' + session.get('email')) and name + '.jpg' in os.listdir('static/' + session.get('email')):
    ...
    if request.method == "POST":
        if 'edit' in request.form:
            ...
            User_().delete_plant_from_user(plant_id, name)
            return redirect("/user_plants")
...
def history(plant_id):
    history = Plants_().get_plant_history(plant_id)

    return render_template("history.html", user=session.get("email", 0), history=history)
...
def create_plant():
    ...
    if request.method == "POST":
        if 'save' in request.form:
            ...
            elif name in Plants_().get_all_plants_names(False):
...
def save_reminder(d):
    ...
    with open('static/tg_data.json', 'r+', encoding='utf-8') as file:
        ...
        addition = {"id": id + 1, "email": session.get("email")[0], "name": d["name"], "m_type": d["m_type"],
                    "tg_id": accounts, "plants": plants, "start_date": info["start_date"], "period": period}
...
def get_emails_api():
    ...
    cursor.execute("SELECT email FROM auth_data")
    res = cursor.fetchall()
    cursor.close()
...
def get_reminders():
    with open("static/tg_data.json", 'r', encoding='utf-8') as file:
        ...
        for item in data:
            if item["email"] == session.get("email")[0]:
